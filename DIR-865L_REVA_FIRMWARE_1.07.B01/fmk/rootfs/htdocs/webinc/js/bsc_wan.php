...
Page.prototype =
{
			...
			if (COMM_Equal(OBJ("rgmode").getAttribute("modified"), true))
			{
				...
				if (OBJ("rgmode").checked)
				{
					if (this.bridge_addrtype==="static")
					{
						...
						var msgArray =
						[
							'<?echo i18n("The device is changing to Bridge Mode.");?>',
							'<?echo i18n("You can access the device by clicking the link below.");?>',
							url
						];
						return_url = "http://"+this.bridge_ipaddr+"/bsc_wan.php";
					}
					else
					{
						...
						[
							"<?echo i18n("The device is changing to Bridge mode.");?>",
							"<?echo i18n("It's rebooting now and management IP address will be changing to 192.168.0.50.");?>",
							"<?echo i18n("Please make sure PC's IP address is in the same IP networking before doing further configuration of device.");?>"
						];
						return_url = "http://192.168.0.50/bsc_wan.php";
					}
				}
				else
				{
					...
					return_url = "http://<?echo $ipaddr;?>/bsc_wan.php";
				}
				BODY.ShowCountdown('<?echo i18n("Device Mode");?>', msgArray, this.bootuptime, return_url);
			}
	...
	PreSubmit: function()
	{
		...
		//+++ Jerry Kao, add NAT-1 for WAN-1 (DS-Lite mode will clear /nat).
		XS(this.wan1.infp+"/nat",         "NAT-1");
	...
	PreDSLite: function()
	{
		...
		else
		{
			...
			if(wanmode6 !="" && wanactive6 !="0") 
			{
				...
				{
					BODY.ShowAlert("<?echo i18n("Please choose one IPv6 connection type besides Link-Local Only, 6to4, 6rd, and IPv6 in IPv4 Tunnel !");?>");
					return false;
				}
				XS(this.wan1.infp+"/infprevious", "WAN-4");
				XS(this.wan4.infp+"/infnext", "WAN-1");
	...
	OnClickMacButton: function(objname)
	{
		...
		if(OBJ(objname).value == "")
			BODY.ShowAlert("<?echo i18n("Can't find Your PC's MAC Address, please enter Your MAC manually.");?>");
	},
