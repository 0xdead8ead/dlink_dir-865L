...
function generate_configs($phyinfuid, $output)
{
	...
	*/
	/*the ssid is content mac now.do not append mac again*/
...
{//2.4G
	...
	if($uid=="BAND24G-1.2" && isfile("/var/run/BAND24G-1.1.DOWN")==1) continue; //host guest dependency check
	$dev = devname($uid);
...
{//5G
	...
	if($cnt<=0) continue;
	if(sta_mode($uid) == "1") continue;
	if($uid=="BAND5G-1.2" && isfile("/var/run/BAND5G-1.1.DOWN")==1) continue; //host guest dependency check
	$dev = devname($uid);
	$cfile = '/var/run/hostapd-'.$dev.'.conf';
