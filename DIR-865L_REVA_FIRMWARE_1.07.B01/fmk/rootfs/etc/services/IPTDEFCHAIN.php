...
function lan_default($infp, $stsp, $name, $dev)
{
	...
	while ($i > 0)
	{
		...
		if($waninf != "")
		{
			startcmd("iptables -t filter -A FORWARD -i ".$dev." -o ".$waninf.
...
//marco, for cone	
	...
	if ($mtu >= 40)
	{
		$mss = $mtu-40;
		$mss1 = $mss+1;
		//set -mss from mss1-60000, some client will send TCP-sync with big mss, so bigger is safer. (tom, 20131126)
		$iptopt = "-p tcp --tcp-flags SYN,RST,FIN SYN -m tcpmss --mss ".$mss1.":60000 -j TCPMSS --set-mss ".$mss;
	...
	if(query($infp."/lowerlayer")=="")
	{
		startcmd("echo --all > /proc/nf_conntrack_flush");
	}
	else
	{
